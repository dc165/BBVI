...
# Follow up data
load("Raw data/35481-0009-Data.rda")
...
X_ind_idx = c("STUDENTID", "Y1SCHOOLID", "FAST", "Q2A", "Q2B", "Q17A", "Q17B", "A5A", "DOB", "GENDER", "RACE_ETHNICITY", "LUNCH_STATUS", "ELL_STATUS", "SPED_STATUS", "KINDERGARTEN", "ATTENDANCE_RATE", "P2", "P3A", "P3B", "P3C", "P4A", "P4B", "P4C", "P6", "P7F")
...
# Merge with FAST and FASTWORKS attendance data
X_ind = left_join(X_ind, da35481.0006, by = "STUDENTID")
X_ind = left_join(X_ind, da35481.0009, by = "STUDENTID")
X_ind = rename(X_ind, Y1SCHOOLID = Y1SCHOOLID.x)
X_ind = mutate(X_ind, Y1SCHOOLID.y = NULL)
...
# Create FASTWORKS features
X_ind$FASTWORKS_ATTEND_TOTAL = rowSums(X_ind[,paste0("S", 1:28)] - 1)

```

```{r}
FAST_avg = X_ind %>% group_by(Y1SCHOOLID) %>% 
  summarize(FAST_avg = mean(na.omit(FAST_ATTEND_RATE)))
FAST_avg = FAST_avg[,2]
X_group = cbind(X_group, FAST_avg)
```



```{r}
# Group level FASTWORKS features
fw_group = X_ind %>% 
  group_by(Y1SCHOOLID) %>% 
  summarise(TOTALSESSIONSHELD = first(TOTALSESSIONSHELD),
            FASTWORKS_AVG_ATTEND = sum(FASTWORKS_ATTEND_TOTAL / TOTALSESSIONSHELD)) %>%
  mutate(across(everything(), ~replace_na(., 0)))
X_group = left_join(X_group, fw_group, by = "Y1SCHOOLID")
```
...
# PCA
library(psych)
pca_res = pca(cor(X_group[,-c(1, 11, 12, 13, 14)]), nfactors = 5)
pca_res$loadings
```
```{r}
# Community variables
pca_parent = pca(cor(X_ind[,c("P2", "P3A", "P3B", "P3C", "P4A", "P4B", "P4C", "P6", "P7F")]), nfactors = 5)
pca_parent$loadings
```
```{r}
# Demographic variables
pca_dem = pca(cor(X_ind[,c("Q2A", "Q2B", "Q17A", "Q17B", "A5A", "DOB", "GENDER", "RACE_HISP", "LUNCH_STATUS", "ELL_STATUS", "SPED_STATUS")]), nfactors = 7)
pca_dem$loadings
```
```{r}
# Selected features
pca_select = pca(cor(X_ind[,c("Q2A", "Q2B", "Q17B", "GENDER", "LUNCH_STATUS", "ELL_STATUS", "SPED_STATUS", "P2", "P3A", "P3C", "P6", "P7F", "FAST_ATTEND_RATE", "FASTWORKS_ATTEND_TOTAL")]), nfactors = 8)
pca_select$loadings
```
...
X_idx_2 = c("Q2A", "Q2B", "Q17B", "GENDER", "LUNCH_STATUS", "ELL_STATUS", "SPED_STATUS", "P2", "P3A", "P3C", "P6", "P7F", "FAST_ATTEND_RATE", "FASTWORKS_ATTEND_TOTAL")
...
U_idx = c("FAST_avg", "FASTWORKS_AVG_ATTEND", "P_RE_HISP", "PHOENIX")
...
X_idx_2 = c("Q17B", "LUNCH_STATUS", "ELL_STATUS", "SPED_STATUS", "P7F")
...
data = data_SCCD
save(data, file = "TDCM_empirical_sig.RData")
```
