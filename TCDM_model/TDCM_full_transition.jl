...
struct TDCModel{T <: AbstractFloat}
    # data
    obs                 :: TDCMObs
    # Dictionaries for indexing skill transition types
    idx_to_transition   :: Vector{Dict{Int, Vector{Int}}}
    ...
    # Group all parameters together for optimizer
...
function TDCModel(
    ...
    # Generate enumerated skill transition dictionaries
    idx_to_transition = Vector{Dict{Int, Vector{Int}}}(undef, O)
    for t in 2:O
        idx_to_transition[t] = Dict((p => digits(p - 1, base = 2, pad = t - 1)) for p in 1:(2^(t-1)))
    end
    ...
    # Group parameters together for optimizer
    params = [pi_star, mu_beta_star, V_beta_star, mu_gamma_star, V_gamma_star, mu_omega_star, V_omega_star, b_tau_star]
    ...
    for k in 1:K
        ...
        for t in 1:O
            if t == 1
                ...
                num_features = size(obs.X[k][t], 2)
                gamma_sample[k][t] = Vector{Vector{Vector{Vector{T}}}}(undef, 2^(t-1))
                for z in 1:(2^(t-1))
    ...
    for k in 1:K
        ...
        for t in 1:O
            ...
            if t == 1
                ...
                omega_sample[k][t] = Vector{Vector{Vector{Vector{T}}}}(undef, 2^(t-1))
                tau_sample[k][t] = Vector{Vector{Vector{T}}}(undef, 2^(t-1))
                for z in 1:(2^(t-1))
    ...
    TDCModel(obs, idx_to_transition, mu_beta_prior, L_beta_prior, mu_omega_prior, L_omega_prior, a_tau_prior, b_tau_prior, enable_parallel,
    pi_star, mu_beta_star, V_beta_star, mu_gamma_star, V_gamma_star, mu_omega_star, V_omega_star, a_tau_star, b_tau_star, params, M,
...
# Object for storing optimizer parameters
struct ADAMparams{T <: AbstractFloat}
    t                   :: Vector{Vector}
    b1                  :: T
    b2                  :: T
    m_t                 :: Vector{Vector}
...
function ADAMparams(
    ...
    t = deepcopy(model.params)
    m_t = deepcopy(model.params)
    v_t = deepcopy(model.params)
    deepfill!(t, 1)
    deepfill!(m_t, 0)
    deepfill!(v_t, 0)
    ADAMparams(t, b1, b2, m_t, v_t, eps)
end
...
function sample_Z(
    ...
    Z_it_variational_distribution = Multinomial(1, model.pi_star[idx_student][idx_skill])
    # Populate preallocated arrays with samples from variational distribution
    rand!(Z_it_variational_distribution, model.Z_sample[idx_student][idx_skill])
end
...
function sample_τ(
    ...
    tau_ktzm_variational_distribution = InverseGamma(model.a_tau_star[idx_skill][idx_time][idx_transition][idx_feature],
                                            model.b_tau_star[idx_skill][idx_time][idx_transition][idx_feature])
    # Populate preallocated arrays with samples from variational distribution
    rand!(tau_ktzm_variational_distribution, model.tau_sample[idx_skill][idx_time][idx_transition][idx_feature])
end
...
function update_categorical_variational_distribution(
    ...
    eps                 :: T = 1e-8,
    verbose             :: Bool = true,
    ADAMparams          :: Union{ADAMparams, Nothing}=nothing
) where T <: AbstractFloat
    obs = model.obs
    ...
    if !model.enable_parallel
        @inbounds for i in 1:N
            ...
            # Get parameters for variational distribution of skill of i-th student at time point 1
            ...
            @inbounds for iter in 1:maxiter
                ...
                @inbounds for m in 1:M
                    ...
                    for k in 1:K
                        ...
                        obs_X = obs.X[k][1][i]
                        if time > 1
                            obs_X = obs.X[k][time][i, :]
                            for o in 1:(time - 1)
                                transition_idx += obs.skill_dict[argmax(Z_sample[i][o][m])][k] * 2^(o-1)
                            end
                        end
                ...
                a_hat = (S_fh - 1/M * SS_fh) / (S_hh - 1/M * SS_hh + eps)
    ...
    if !model.enable_parallel
        @inbounds for j in 1:J
            ...
            @inbounds for iter in 1:maxiter
                ...
                copy!(C_star_old_j, V_star_old_j)
                ...
                ELBO = 0
                # Set V_star_old_j to V* by multiplying cholesky factors
                # This is required since sample_γ uses the memory at V_star_old_j as covariance matrix
                ...
                # Adjust storage_gradC_m to account for log transformation of diagonal
                # log transform diagonal of cholesky factor of covariance matrix
                for k in 1:len_beta
                    # Diagonal indices of matrix after half vectorization
                    idx = Int(len_beta * (k - 1) - (k - 2) * (k - 1) / 2 + 1)
                    storage_gradC_m[idx] /= C_star_old_j[k, k]
                    C_star_old_j[k, k] = log(C_star_old_j[k, k])
                end
                ...
                if !isnothing(ADAMparams)
                    # ADAM optimization update
                    t_ADAM = ADAMparams.t[2][j][1]
                    m_t_mu = ADAMparams.m_t[2][j]
                    m_t_V = view(ADAMparams.m_t[3][j], [len_beta * (j - 1) + i for j in 1:len_beta for i in j:len_beta])
                    v_t_mu = ADAMparams.v_t[2][j]
                    v_t_V = view(ADAMparams.v_t[3][j], [len_beta * (j - 1) + i for j in 1:len_beta for i in j:len_beta])
                    b1_t = ADAMparams.b1 ^ t_ADAM
                    b2_t = ADAMparams.b2 ^ t_ADAM
                    ADAMparams.t[2][j][1] += 1
                    m_t_mu .= ADAMparams.b1 .* m_t_mu + (1 - ADAMparams.b1) .* grad_mu_L
                    m_t_V .= ADAMparams.b1 .* m_t_V .+ (1 - ADAMparams.b1) .* vech_grad_C_L
...
function update_normal_variational_distribution2(
    ...
    if !model.enable_parallel
        @inbounds for k in 1:K
            for t in 1:O
                for z in 1:(2^(t - 1))
                    ...
                    for s in 1:S
                        mu_star_old_j = mu_star_old[k][t][z][s]
                        V_star_old_j = V_star_old[k][t][z][s]
                        # Perform gradient descent update of mu_j and V_j
                        ...
                        grad_mu_L_m = view(model.storage_grad_L, 1:len_gamma)
                        ...
                        vech_grad_C_L_m = view(model.storage_grad_LL, 1:Int(len_gamma*(len_gamma + 1)/2))
                        grad_mu_log_q = view(model.storage_L2, 1:len_gamma)
                        grad_mu_log_q_m = view(model.storage_grad_q_L, 1:len_gamma)
                        ...
                        L_gamma_minus_mu = view(model.storage_L4, 1:len_gamma)
                        ...
                        storage_gradC_m = view(model.storage_grad_q_LL, 1:Int(len_gamma * (len_gamma + 1) / 2))
                        # Generate commutation and duplication matrix
                        ...
                        # step_iterator = step_iterator_factory(init_step)
                        # Storage for intermediate terms in control variate calculation
                        S_f_mu = view(model.storage_S_f_L, 1:len_gamma)
                        S_h_mu = view(model.storage_S_h_L, 1:len_gamma)
                        S_f_C = view(model.storage_S_f_LL, 1:Int(len_gamma * (len_gamma + 1) / 2))
                        S_h_C = view(model.storage_S_h_LL, 1:Int(len_gamma * (len_gamma + 1) / 2))
                        @inbounds for iter in 1:maxiter
                            # Set gradient to zero
                            fill!(grad_mu_L, 0)
                            fill!(grad_C_L, 0)
                            ...
                            # Calculate the gradient estimate of the m-th sample
                            @inbounds for m in 1:M
                                gamma_ktzsm = gamma_sample[k][t][z][s][m]
                                fill!(grad_mu_log_q_m, 0)
                                # grad_mu_log_q = Vinv_star * β_jm
                                BLAS.gemv!('N', T(1), Vinv_star_old_j, gamma_ktzsm, T(1), grad_mu_log_q_m)
                                ...
                                BLAS.gemv!('T', T(1), dup_j, storage_len_gamma_sqr2, T(1), fill!(storage_gradC_m, 0))
                                # Calculate log(p(Y, β_(j)))
                                log_prob_Ygamma = 0
                                for i in 1:N
                                    if obs.group[i] != s
                                    ...
                                    log_prob_Ygamma += log(sigmoid((2*skill_profile[k] - 1) * 
                                        dot(gamma_sample[k][t][z][s][m], obs.X[k][t][i, :])))
                                end
                                num_features = length(gamma_ktzsm)
                                for feature in 1:num_features
                                    tau = tau_sample[k][t][z][feature][m]
                                    omega = omega_sample[k][t][z][feature][m]
                                    u = obs.U[k][t][s, :]
                                    log_prob_Ygamma += - 1/(2 * tau) * (gamma_ktzsm[feature] - dot(u, omega))^2
                                end
                                gamma_minus_mu .= gamma_ktzsm
                                gamma_minus_mu .-= mu_star_old_j
                                log_q = -len_gamma / 2 * log(2 * pi) - 1 / 2 * logdet_V_j - 1 / 2 * dot(gamma_minus_mu, grad_mu_log_q_m)
                                # Update average gradient
                                grad_mu_L_m .= grad_mu_log_q_m .* (log_prob_Ygamma - log_q)
                                vech_grad_C_L_m .= storage_gradC_m .* (log_prob_Ygamma - log_q)
                                grad_mu_L .= (m - 1) / m .* grad_mu_L + 1 / m .* grad_mu_L_m
                                vech_grad_C_L .= (m - 1) / m .* vech_grad_C_L + 1 / m .* vech_grad_C_L_m
                                # Update average variational gradient
                                grad_mu_log_q .= (m - 1) / m .* grad_mu_log_q .+ 1 / m .* grad_mu_log_q_m
                            ...
                            # grad_mu_L_norm = norm(grad_mu_L)
                            # if grad_mu_L_norm > clip * sqrt(len_gamma)
                            #     grad_mu_L .= sqrt(len_gamma) * clip .* grad_mu_L ./ grad_mu_L_norm
                            # end
                            ...
                            for k in 1:len_gamma
                                # Diagonal indices of matrix after half vectorization
                                idx = Int(len_gamma * (k - 1) - (k - 2) * (k - 1) / 2 + 1)
                                storage_gradC_m[idx] /= C_star_old_j[k, k]
                                C_star_old_j[k, k] = log(C_star_old_j[k, k])
                            end
                            # Update mu and C with one step
                            ...
                            if !isnothing(ADAMparams)
                                # ADAM optimization update
                                t_ADAM = ADAMparams.t[4][k][t][z][s][1]
                                m_t_mu = ADAMparams.m_t[4][k][t][z][s]
                                m_t_V = view(ADAMparams.m_t[5][k][t][z][s], [len_gamma * (j - 1) + i for j in 1:len_gamma for i in j:len_gamma])
                                v_t_mu = ADAMparams.v_t[4][k][t][z][s]
                                v_t_V = view(ADAMparams.v_t[5][k][t][z][s], [len_gamma * (j - 1) + i for j in 1:len_gamma for i in j:len_gamma])
                                b1_t = ADAMparams.b1 ^ t_ADAM
                                b2_t = ADAMparams.b2 ^ t_ADAM
                                ADAMparams.t[4][k][t][z][s][1] += 1
                                m_t_mu .= ADAMparams.b1 .* m_t_mu + (1 - ADAMparams.b1) .* grad_mu_L
                                m_t_V .= ADAMparams.b1 .* m_t_V .+ (1 - ADAMparams.b1) .* vech_grad_C_L
                                v_t_mu .= ADAMparams.b2 .* v_t_mu .+ (1 - ADAMparams.b2) .* grad_mu_L .^2
                                v_t_V .= ADAMparams.b2 .* v_t_V .+ (1 - ADAMparams.b2) .* vech_grad_C_L .^2
                                ...
                                vech_grad_C_L .= sqrt(1 - b2_t) / (1 - b1_t) .* m_t_V ./ (sqrt.(v_t_V) .+ ADAMparams.eps)
                                vech_grad_C_L_norm = norm(vech_grad_C_L)
                                if vech_grad_C_L_norm > clip * sqrt(len_gamma*(len_gamma+1)/2)
                                    vech_grad_C_L .= sqrt(len_gamma) * clip .* vech_grad_C_L ./ vech_grad_C_L_norm
                                end
        ...
        Threads.@threads for idx in collect(Iterators.flatten(Iterators.product(1:K, t, 1:2^(t - 1), 1:S) for t in 1:O))
            k, t, z, s = idx[1], idx[2], idx[3], idx[4]
            # Get thread id
            tid = Threads.threadid()
            mu_star_old_j = mu_star_old[k][t][z][s]
            V_star_old_j = V_star_old[k][t][z][s]
            ...
            grad_mu_L_m = view(model.storage_grad_L_par[tid], 1:len_gamma)
            grad_C_L = view(model.storage_LL2_par[tid], 1:len_gamma, 1:len_gamma)
            vech_grad_C_L = view(grad_C_L, [len_gamma * (j - 1) + i for j in 1:len_gamma for i in j:len_gamma]) # Uses same memory as grad_C_L
            vech_grad_C_L_m = view(model.storage_grad_LL_par[tid], 1:Int(len_gamma*(len_gamma + 1)/2))
            grad_mu_log_q = view(model.storage_L2_par[tid], 1:len_gamma)
            grad_mu_log_q_m = view(model.storage_grad_q_L_par[tid], 1:len_gamma)
            ...
            L_gamma_minus_mu = view(model.storage_L4_par[tid], 1:len_gamma)
            ...
            storage_gradC_m = view(model.storage_grad_q_LL_par[tid], 1:Int(len_gamma * (len_gamma + 1) / 2))
            # Generate commutation and duplication matrix
            ...
            # step_iterator = step_iterator_factory(init_step)
            # Storage for intermediate terms in control variate calculation
            S_f_mu = view(model.storage_S_f_L_par[tid], 1:len_gamma)
            S_h_mu = view(model.storage_S_h_L_par[tid], 1:len_gamma)
            S_f_C = view(model.storage_S_f_LL_par[tid], 1:Int(len_gamma * (len_gamma + 1) / 2))
            S_h_C = view(model.storage_S_h_LL_par[tid], 1:Int(len_gamma * (len_gamma + 1) / 2))
            @inbounds for iter in 1:maxiter
                # Set gradient to zero
                fill!(grad_mu_L, 0)
                fill!(grad_C_L, 0)
                ...
                logdet_V_j = 0
                for b in 1:len_gamma
                    logdet_V_j += 2 * log(abs(Vinv_star_old_j[b, b]))
                end
                # Copy lower triangular cholesky factor into preallocated storage
                for k in 1:len_gamma
                    for l in 1:k
...
function update_normal_variational_distribution3(
    ...
    # Fully update parameters of each β_j using noisy gradients before moving to update parameters of next β_j
    if !model.enable_parallel
        @inbounds for idx in Iterators.flatten(Iterators.product(1:K, t, 1:(2^(t-1))) for t in 1:O)
            ...
            for feature in 1:num_features
                ...
                grad_mu_L_m = view(model.storage_grad_L, 1:len_omega)
                ...
                vech_grad_C_L_m = view(model.storage_grad_LL, 1:Int(len_omega*(len_omega + 1)/2))
                grad_mu_log_q = view(model.storage_L2, 1:len_omega)
                grad_mu_log_q_m = view(model.storage_grad_q_L, 1:len_omega)
                ...
                storage_gradC_m = view(model.storage_grad_q_LL, 1:Int(len_omega * (len_omega + 1) / 2))
                # Generate commutation and duplication matrix
                ...
                # step_iterator = step_iterator_factory(init_step)
                # Storage for intermediate terms in control variate calculation
                S_f_mu = view(model.storage_S_f_L, 1:len_omega)
                S_h_mu = view(model.storage_S_h_L, 1:len_omega)
                S_f_C = view(model.storage_S_f_LL, 1:Int(len_omega * (len_omega + 1) / 2))
                S_h_C = view(model.storage_S_h_LL, 1:Int(len_omega * (len_omega + 1) / 2))
                for iter in 1:maxiter
                    ...
                    LinearAlgebra.copytri!(Vinv_star_old_j, 'L')
                    # Set V_star_old_j to V* by multiplying cholesky factors
                    # This is required since sample_ω uses the memory at V_star_old_j as covariance matrix
                    BLAS.gemm!('N', 'T', T(1), V_star_old_j, V_star_old_j, T(1), fill!(grad_V_log_q, 0))
                    copy!(V_star_old_j, grad_V_log_q)
                    # Sample ω from variational distribution
                    ...
                    # Calculate the gradient estimate of the m-th sample
                    for m in 1:M
                        omega_ktzm = omega_sample[k][t][z][feature][m]
                        fill!(grad_mu_log_q_m, 0)
                        # grad_mu_log_q_m = Vinv_star * β_jm
                        BLAS.gemv!('N', T(1), Vinv_star_old_j, omega_ktzm, T(1), grad_mu_log_q_m)
                        # grad_mu_log_q_m = Vinv_star_j * β_jm - Vinv_star_j * mu_star_j
                        BLAS.gemv!('N', T(-1), Vinv_star_old_j, mu_star_old_j, T(1), grad_mu_log_q_m)
                        # grad_V_log_q = -1/2(Vinv_star_j - Vinv_star_j * (β_jm - mu_star_j) * (β_jm - mu_star_j)^T * Vinv_star_j)
                        copy!(grad_V_log_q, Vinv_star_old_j)
                        ...
                        # storage_gradC_m = D'((C_j ⊗ I)' + K'(I ⊗ C_j)')vec(grad_V_log_q)
                        BLAS.gemv!('T', T(1), dup_j, storage_len_omega_sqr2, T(1), fill!(storage_gradC_m, 0))
                        # Calculate log(p(Y, β_(j)))
                        log_prob_Yomega = 0
                        for s in 1:S
                            tau = tau_sample[k][t][z][feature][m]
                            gamma = gamma_sample[k][t][z][s][m][feature]
                            u = obs.U[k][t][s, :]
                            log_prob_Yomega += - 1/(2 * tau) * (gamma - dot(u, omega_ktzm))^2
                        end
                        omega_minus_mu .= omega_ktzm
                        omega_minus_mu .-= model.mu_omega_prior[k][t][z][feature]
                        mul!(L_omega_minus_mu, model.L_omega_prior[k][t][z][feature], omega_minus_mu)
                        log_prob_Yomega -= 1/2 * dot(L_omega_minus_mu, L_omega_minus_mu)
                        omega_minus_mu .= omega_ktzm
                        omega_minus_mu .-= mu_star_old_j
                        log_q = -len_omega / 2 * log(2 * pi) - 1 / 2 * logdet_V_j - 1 / 2 * dot(omega_minus_mu, grad_mu_log_q_m)
                        # Update average gradient
                        grad_mu_L_m .= grad_mu_log_q_m .* (log_prob_Yomega - log_q)
                        vech_grad_C_L_m .= storage_gradC_m .* (log_prob_Yomega - log_q)
                        grad_mu_L .= (m - 1) / m .* grad_mu_L + 1 / m .* grad_mu_L_m
                        vech_grad_C_L .= (m - 1) / m .* vech_grad_C_L + 1 / m .* vech_grad_C_L_m
                        # Update average variational gradient
                        grad_mu_log_q .= (m - 1) / m .* grad_mu_log_q .+ 1 / m .* grad_mu_log_q_m
                    ...
                    # grad_mu_L_norm = norm(grad_mu_L)
                    # if grad_mu_L_norm > clip * sqrt(len_omega)
                    #     grad_mu_L .= sqrt(len_omega) * clip .* grad_mu_L ./ grad_mu_L_norm
                    # end

                    # vech_grad_C_L_norm = norm(vech_grad_C_L)
                    # if vech_grad_C_L_norm > clip * sqrt(len_omega*(len_omega+1)/2)
                    #     vech_grad_C_L .= sqrt(len_omega) * clip .* vech_grad_C_L ./ vech_grad_C_L_norm
                    # end
                    ...
                    for k in 1:len_omega
                        # Diagonal indices of matrix after half vectorization
                        idx = Int(len_omega * (k - 1) - (k - 2) * (k - 1) / 2 + 1)
                        storage_gradC_m[idx] /= C_star_old_j[k, k]
                        C_star_old_j[k, k] = log(C_star_old_j[k, k])
                    end
                    # Update mu and C with one step
                    ...
                    if !isnothing(ADAMparams)
                        # ADAM optimization update
                        t_ADAM = ADAMparams.t[6][k][t][z][feature][1]
                        m_t_mu = ADAMparams.m_t[6][k][t][z][feature]
                        m_t_V = view(ADAMparams.m_t[7][k][t][z][feature], [len_omega * (j - 1) + i for j in 1:len_omega for i in j:len_omega])
                        v_t_mu = ADAMparams.v_t[6][k][t][z][feature]
                        v_t_V = view(ADAMparams.v_t[7][k][t][z][feature], [len_omega * (j - 1) + i for j in 1:len_omega for i in j:len_omega])
                        b1_t = ADAMparams.b1 ^ t_ADAM
                        b2_t = ADAMparams.b2 ^ t_ADAM
                        ADAMparams.t[6][k][t][z][feature][1] += 1
                        m_t_mu .= ADAMparams.b1 .* m_t_mu .+ (1 - ADAMparams.b1) .* grad_mu_L
                        m_t_V .= ADAMparams.b1 .* m_t_V .+ (1 - ADAMparams.b1) .* vech_grad_C_L
                        v_t_mu .= ADAMparams.b2 .* v_t_mu .+ (1 - ADAMparams.b2) .* grad_mu_L .^2
                        v_t_V .= ADAMparams.b2 .* v_t_V .+ (1 - ADAMparams.b2) .* vech_grad_C_L .^2
        ...
        Threads.@threads for idx in collect(Iterators.flatten(Iterators.product(1:K, t, 1:(2^(t-1))) for t in 1:O))
            ...
            for feature in 1:num_features
                ...
                grad_mu_L_m = view(model.storage_grad_L_par[tid], 1:len_omega)
                grad_C_L = view(model.storage_LL2_par[tid], 1:len_omega, 1:len_omega)
                vech_grad_C_L = view(grad_C_L, [len_omega * (j - 1) + i for j in 1:len_omega for i in j:len_omega]) # Uses same memory as grad_C_L
                vech_grad_C_L_m = view(model.storage_grad_LL_par[tid], 1:Int(len_omega*(len_omega + 1)/2))
                grad_mu_log_q = view(model.storage_L2_par[tid], 1:len_omega)
                grad_mu_log_q_m = view(model.storage_grad_q_L_par[tid], 1:len_omega)
                ...
                storage_gradC_m = view(model.storage_grad_q_LL_par[tid], 1:Int(len_omega * (len_omega + 1) / 2))
                # Generate commutation and duplication matrix
                ...
                # step_iterator = step_iterator_factory(init_step)
                # Storage for intermediate terms in control variate calculation
                S_f_mu = view(model.storage_S_f_L_par[tid], 1:len_omega)
                S_h_mu = view(model.storage_S_h_L_par[tid], 1:len_omega)
                S_f_C = view(model.storage_S_f_LL_par[tid], 1:Int(len_omega * (len_omega + 1) / 2))
                S_h_C = view(model.storage_S_h_LL_par[tid], 1:Int(len_omega * (len_omega + 1) / 2))
                @inbounds for iter in 1:maxiter
                    # Set gradient to zero
                    fill!(grad_mu_L, 0)
                    fill!(grad_C_L, 0)
        ...
        @inbounds for idx in Iterators.flatten(Iterators.product(1:K, t, 1:(2^(t-1))) for t in 1:O)
            ...
            for feature in 1:num_features
                for iter in 1:maxiter
                    ...
                    a_star = model.a_tau_star[k][t][z][feature]
                    b_star = model.b_tau_star[k][t][z][feature]
                    # log_a = log(a_star)
                    log_b = log(b_star)
                    loggamma_a = loggamma(a_star)
                    # digamma_a = digamma(a_star)
                    for m in 1:M
                        tau_ktzm = tau_sample[k][t][z][feature][m]
                        ...
                        a_prior = model.a_tau_prior[k][t][z][feature]
                        b_prior = model.b_tau_prior[k][t][z][feature]
                    ...
                    if !isnothing(ADAMparams)
                        # ADAM optimization update
                        t_ADAM = ADAMparams.t[8][k][t][z][feature]
                        m_t_b = ADAMparams.m_t[8][k][t][z][feature]
                        v_t_b = ADAMparams.v_t[8][k][t][z][feature]
                        b1_t = ADAMparams.b1 ^ t_ADAM
                        b2_t = ADAMparams.b2 ^ t_ADAM
                        ADAMparams.t[8][k][t][z][feature] += 1
                        m_t_b = ADAMparams.b1 * m_t_b + (1 - ADAMparams.b1) * grad_b_L
                        v_t_b = ADAMparams.b2 * v_t_b + (1 - ADAMparams.b2) * grad_b_L .^2
                        log_b += step * sqrt(1 - b2_t) / (1 - b1_t) * m_t_b / (sqrt(v_t_b) + ADAMparams.eps)
                    else
                        # SGD update
                        # log_a += step * grad_a_L
                        log_b += step * grad_b_L
                    end
                    # model.a_tau_star[k][t][z][feature] = exp(log_a)
                    model.b_tau_star[k][t][z][feature] = exp(log_b)
                end
            end
        end
    else
