...
function DCModel(
    ...
    b0              :: T,
    M               :: Int;
    enable_parallel :: Bool=false) where T <: AbstractFloat
    ...
    DCModel(obs, d0, a0, b0, enable_parallel,
...
function update_pi_star_parallel(
    model           :: DCModel;
    step            :: T = 1e-2,
    tol             :: T = 1e-6,
    maxiter         :: Int = 100000,
    ...
    @inbounds for i in 1:N
        ...
        @inbounds for iter in 1:maxiter
            ...
            Threads.@threads for m in 1:M
                ...
                lock(lk) do
                    # Update average gradient
                    grad_L .= (curr_m[1] - 1)/curr_m[1] .* grad_L + 1/curr_m[1] .* (z_im .- pi_star_old_i) .* (log_prob_YZ - log_q)
                    # Update ELBO estimator
                    ELBO = (curr_m[1]-1)/curr_m[1] * ELBO + 1/curr_m[1] * (log_prob_YZ - log_q)
                    # Update counter
                    curr_m[1] += 1
                end
            end
...
function update_mu_star_V_star_parallel(
    model       :: DCModel;
    init_step   :: T=1e-3,
